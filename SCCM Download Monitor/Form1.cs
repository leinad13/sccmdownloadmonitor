...
namespace SCCM_Download_Monitor
{
	public partial class Form1 : Form
	{
		...
		private async void btnReboot_Click(object sender, EventArgs e)
		{
			...
			if (PingMachine(tbHostname.Text))
			{
                Reboot reboot = await GetPendingReboot(tbHostname.Text);
                if (reboot != null)
                {
                    chkCBServicing.Checked = reboot.CBServicing;
                    chkCCM.Checked = reboot.CCMClientSDK;
                    chkCompRen.Checked = reboot.PendingComputerRename;
                    chkFileRen.Checked = reboot.PendingFileRename;
                    chkWU.Checked = reboot.WindowsUpdate;
                    lbFileRenames.DataSource = reboot.PendingFiles;
                }

            }
		}

		private async Task<Reboot> GetPendingReboot(string hostname)
		{
			return await Task.Run(() =>
			{
				Reboot reboot = new Reboot();

				string script = File.ReadAllText("Get_PendingReboot.ps1");

				using (Runspace space = RunspaceFactory.CreateRunspace())
				{
					space.Open();
					space.SessionStateProxy.SetVariable("computername", tbHostname.Text);
					using (PowerShell ps1 = PowerShell.Create())
					{
						...
						System.Collections.ObjectModel.Collection<PSObject> psOutput = ps1.Invoke();

						foreach (PSObject obj in psOutput)
						{
							reboot.CBServicing = (bool)obj.Members["CBServicing"].Value;
							reboot.CCMClientSDK = (bool)obj.Members["CCMClientSDK"].Value;
							reboot.PendingComputerRename = (bool)obj.Members["PendComputerRename"].Value;
							reboot.WindowsUpdate = (bool)obj.Members["WindowsUpdate"].Value;
							reboot.PendingFileRename = (bool)obj.Members["PendFileRename"].Value;
							reboot.PendingFiles = ((string[])obj.Members["PendFileRenVal"].Value).ToList();
						}
